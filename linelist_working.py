...
def clean_after_run(
    path2reuslts: str, path2linemask: str, truncate_warnings: bool = True
):
    linemask_data = extract_linemask(path2linemask)
    result_data = get_model_data(path2reuslts)
...
def extract_element(path2vald: str, element_name: str) -> np.ndarray:
    data = np.genfromtxt(
        path2vald, delimiter=",", dtype=str, skip_header=3, invalid_raise=False
    )
...
if __name__ == "__main__":
    ...
    path2output = "2025-03-31-16-46-54_0.7936911305143918_LTE_Fe_1D"
    clean_after_run(
        tsfit_output + path2output,
        "/Users/beta/synth_spectrum/linemask_files/Fe/fe1_gleb.txt",
    )
