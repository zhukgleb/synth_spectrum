...
spectrum_names = ["R=60000 inter", "R=40000 inter", "R=15000 inter"]
...
for i in range(len(spectrum_arr)):
    ...
    for j in range(1, 51, 10):
       ...
       mult = 100
       cv, z, _, _ = find_velocity(spectrum_arr[i], [a_template, f_template], [5000, 5100], mult)
       velocity.append(cv)
       z_velocity.append(z)
...
plt.grid()
for i in range(len(spectrum_arr)):
    plt.scatter(total_velocity_data[i][0], total_velocity_data[i][1], label=spectrum_names[i], alpha=0.7)
...
plt.legend()
plt.plot([0, 50], [0, 50], linestyle='dashed', color="gray", alpha=0.5)
plt.show()
