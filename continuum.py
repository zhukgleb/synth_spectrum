...
class AutomaticContinuumFitter:
    ...
    def _find_best_matching_model(self, observed_spec, current_continuum):
        ...
        distances = np.sqrt(
            np.sum((self.normalized_models - normalized_observed) ** 2, axis=1)
        )
    ...
    def fit(self, observed_wavelength, observed_spectrum, initial_continuum=None):
        ...
        for iteration in range(self.n_iterations):
            ...
            if iteration > 0:
                alpha = 0.7  # Emperical...
                current_continuum = (
                    alpha * current_continuum + (1 - alpha) * new_continuum
                )
            else:
            ...
            print(
                f"Iteration {iteration + 1}: Best model index = {best_idx}, "
        ...
        final_continuum_interp = interp1d(
            self.wavelength,
            current_continuum,
            kind="cubic",
            bounds_error=False,
